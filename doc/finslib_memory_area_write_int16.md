...
The function `finslib_memory_area_write_int16()` can be used to write a block of signed 16 bit integer values to a memory
...
Existence and filling of the dataspace is the responsibility of the calling function, but `finslib_memory_area_write_int16()`
...
The number of signed integer values to be written in one function call is not limited by the amount of data a PLC can send in one FINS packet because
`finslib_memory_area_write_int16()` will automatically use multiple request at the FINS layer if the dataset is 
too large.
