...
The function `finslib_memory_area_read_bcd32()` can be used to retrieve a block of 32 bit unsigned BCD values from a memory area in a remote PLC. The connection with the PLC should already be present before this function is called.

Data is transferred unmodified from the FINS data stream to the caller supplied buffer.  Enough dataspace is the responsibility of the calling function, but `finslib_memory_area_read_bcd32()` will return an error if a NULL pointer is provided for data storage.
